...
public class DualityImageState extends DualityState {
  //========== Constructor ===========
  public DualityImageState() {
  ...
  //--- Ending Events ---
  @Override
  public void handle_EVendDuality(Duality object, Context ctx) throws FailedEventHandlingException {

    //Check if there are living  dependents before ending the object
    if(StubHelper.hasLivingDependents(ctx, object))
      throw new FailedEventHandlingException("Duality (" + object.getId() + ") has living dependents and therefore cannot be ended");

    //Changes the state of the current Duality (object)
    DualityArchivedState newState = new DualityArchivedState();
    object.setCurrentState(newState);
    StubHelper.save(ctx, object);
    //Trigger Event Handling for Each Master
    //EconomicEvent
    EconomicEvent economicEvent_Settle_Duality = (EconomicEvent)StubHelper.findBusinessObject(ctx, object.getEconomicEventId_Settle_Duality());
    economicEvent_Settle_Duality.getCurrentState().handle_EVendDuality(economicEvent_Settle_Duality, ctx);
    //EconomicEvent
    EconomicEvent economicEvent_Claim_Duality = (EconomicEvent)StubHelper.findBusinessObject(ctx, object.getEconomicEventId_Claim_Duality());
    economicEvent_Claim_Duality.getCurrentState().handle_EVendDuality(economicEvent_Claim_Duality, ctx);
  }
}
