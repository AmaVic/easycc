...
public class StockFlowUseState extends StockFlowState {
  //========== Constructor ===========
  public StockFlowUseState() {
    super("use", StateType.ONGOING);
  }
  ...
  //--- Modifying Events ---
  @Override
  public void handle_EVcrIncrement(StockFlow object, Context ctx) throws FailedEventHandlingException {
    ...
    economicResource_EconomicResource_StockFlow.getCurrentState().handle_EVcrIncrement(economicResource_EconomicResource_StockFlow, ctx);
    //EconomicEvent
    ...
    economicEvent_EconomicEvent_StockFlow.getCurrentState().handle_EVcrIncrement(economicEvent_EconomicEvent_StockFlow, ctx);
  }
  @Override
  public void handle_EVcrDecrement(StockFlow object, Context ctx) throws FailedEventHandlingException {
    ...
    if(!object.getEconomicEventId_EconomicEvent_StockFlow().equals(currentLedgerObject.getEconomicEventId_EconomicEvent_StockFlow()))
      throw new FailedEventHandlingException("The master of a Business Object (" + object.getId() + ") cannot be changed");
    //Changes the state of the current StockFlow (object)
    StockFlowOutflowState newState = new StockFlowOutflowState();
    object.setCurrentState(newState);
    StubHelper.save(ctx, object);
    //Trigger Event Handling for Each Master
    //EconomicResource
  ...
  //--- Ending Events ---
  @Override
  public void handle_EVendStockFlow(StockFlow object, Context ctx) throws FailedEventHandlingException {

    //Check if there are living  dependents before ending the object
    if(StubHelper.hasLivingDependents(ctx, object))
      throw new FailedEventHandlingException("StockFlow (" + object.getId() + ") has living dependents and therefore cannot be ended");

    //Changes the state of the current StockFlow (object)
    StockFlowArchivedState newState = new StockFlowArchivedState();
    object.setCurrentState(newState);
    StubHelper.save(ctx, object);
    //Trigger Event Handling for Each Master
    //EconomicResource
    EconomicResource economicResource_EconomicResource_StockFlow = (EconomicResource)StubHelper.findBusinessObject(ctx, object.getEconomicResourceId_EconomicResource_StockFlow());
    economicResource_EconomicResource_StockFlow.getCurrentState().handle_EVendStockFlow(economicResource_EconomicResource_StockFlow, ctx);
    //EconomicEvent
    EconomicEvent economicEvent_EconomicEvent_StockFlow = (EconomicEvent)StubHelper.findBusinessObject(ctx, object.getEconomicEventId_EconomicEvent_StockFlow());
    economicEvent_EconomicEvent_StockFlow.getCurrentState().handle_EVendStockFlow(economicEvent_EconomicEvent_StockFlow, ctx);
  }
}
