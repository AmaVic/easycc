...
public class BusinessEvent {
  ...
  private BusinessObject handleCreatingEvent(Context ctx, String jsonPayload) throws FailedEventHandlingException {
    ...
    //20230802 Enforce Uniqueness Constrait here by throwing an exception before the Context containing the new business object can be saved.
    //retrieve the uniqueness query based on the dynamic type of the Business Object (e.g. Stockflow)
    String query = builtFromJson.getUniquenessQuery();
    System.out.println("Uniqueness query: "+query);//for debuggin only
    //Query the context for similar object
    QueryResultsIterator<KeyValue> results = ctx.getStub().getQueryResult(query);
    System.out.println("Uniqueness query results: "+results);//for debuggin only
    //Raise exception in case the Query returns a result
    if(results.iterator().hasNext()){System.out.println("Uniqueness exception to be thrown because of object"+this.name);} //for debuggin only
    else{System.out.println(this.name+" is unique");}//for debuggin only
    if(results.iterator().hasNext())
      throw new FailedEventHandlingException("This object's combination of Master objects is not unique");
